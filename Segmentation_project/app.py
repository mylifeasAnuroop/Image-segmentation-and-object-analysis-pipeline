...
# Define paths using dynamic resolution
output_path = os.path.join('data', 'output')
segmented_objects_path = os.path.join('data', 'segmented_objects')
final_metadata_file = os.path.join(output_path, 'final_metadata.json')
...
if os.path.exists(final_metadata_file):
    ...
    # Iterate through each image and its associated metadata
    for image_name, metadata in final_metadata.items():
        st.subheader(f"Analysis Results for {image_name}")
        
        # Display the master image
        master_image_path = os.path.join(project_root, metadata['master_image'])
        if os.path.exists(master_image_path):
        ...
        for obj in metadata['segmented_objects']:
            ...
            object_image_path = os.path.join(project_root, obj['object_image'])
            summary_table_path = os.path.join(project_root, obj['summary_table'])
