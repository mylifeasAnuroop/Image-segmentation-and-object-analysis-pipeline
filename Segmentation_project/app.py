...
# Navigation options
page = st.sidebar.radio("Select a page:", ["Upload", "View Results"])
...
if page == "Upload":
    ...
    if uploaded_file:
        image_path = os.path.join(input_images_path, uploaded_file.name)
        ...
        if st.button("Process Image"):
            ...
            run_segmentation(image_path, segmented_objects_path)
            run_identification(image_path, segmented_objects_path)
            run_text_extraction(image_path)
            run_summarization(image_path)
            map_data(image_path, metadata_file)
    ...
    if os.path.exists(final_metadata_file):
        ...
        for image_name, data in final_metadata.items():
            st.subheader(f"Results for {image_name}")

            st.image(os.path.join('data', 'output', data["master_image"]), caption="Annotated Master Image", use_column_width=True)
            
            for obj in data["segmented_objects"]:
                st.image(os.path.join('data', 'segmented_objects', os.path.basename(obj["object_image"])), caption="Segmented Object Image", use_column_width=True)
                st.image(os.path.join('data', 'output', os.path.basename(obj["summary_table"])), caption="Summary Table", use_column_width=True)
                
    else:
        st.warning("No results available. Please upload and process an image first.")
